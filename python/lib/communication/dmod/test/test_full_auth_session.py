...
class TestFullAuthSession(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_factory_init_from_deserialized_json_0_b(self):
        """
        Assert that factory init method for deserializing produces an equal object to the pre-created object for the
        examples at the 0th index, using more thorough checking than just the default for the type (which is predecated
        on the session id).
        """
        example_index = 0
        obj = self.tested_serializeable_type.factory_init_from_deserialized_json(self.request_jsons[example_index])
        self.assertTrue(self.tested_serializeable_type.full_equals(obj, self.request_objs[example_index]))

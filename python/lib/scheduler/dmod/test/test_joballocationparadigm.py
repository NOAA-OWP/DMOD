import unittest
from ..scheduler.job.job import JobAllocationParadigm
...
class TestJobAllocationParadigm(unittest.TestCase):
    ...
    def test_get_default_selection_1_a(self):
        default_val = JobAllocationParadigm.get_default_selection()
        default_sch_msg_str = SchedulerRequestMessage.default_allocation_paradigm_str()
        default_msg_val = JobAllocationParadigm.get_from_name(default_sch_msg_str, strict=True)
    ...
    def test_get_from_name_1_a(self):
        ...
        parse_value = JobAllocationParadigm.get_from_name(test_name, strict=False)
        self.assertEqual(parse_value, JobAllocationParadigm.get_default_selection())

    # Test that a bogus name returns None when strict
    def test_get_from_name_1_b(self):
        test_name = 'blah_blah_blah'
        parse_value = JobAllocationParadigm.get_from_name(test_name, strict=True)
    ...
    def test_get_from_name_2_a(self):
        base_type = JobAllocationParadigm.SINGLE_NODE
    ...
    def test_get_from_name_3_a(self):
        base_type = JobAllocationParadigm.FILL_NODES

...
class Launcher(SimpleDockerUtil):
    ...
    def _generate_nextgen_job_docker_cmd_args(self, job: 'Job', worker_index: int) -> Dict[str, str]:
        ...
        Nextgen-based job executed within Docker, beyond CMD args universal to all jobs.  In general, this function
        should not be used except when called by ::method:`_generate_docker_cmd_args`. Further, it only applies to
        Nextgen-based jobs:  i.e., ngen model exec or ngen calibration jobs.
        ...
        -------
        Dict[str, str]
            The Docker CMD param flags and values.
        ...
        ngen_cmd_arg_map = {
            "--output-dataset": self._ds_names_helper(job, worker_index, DataCategory.OUTPUT, max_count=1)[0],
        ...
        if job.cpu_count > 1:
            ...
            ngen_cmd_arg_map["--partition-dataset"] = partition_config_dataset_names[0]
        ...
        # TODO: confirm this properly accounts for differences between ngen exec and calibration jobs

        return ngen_cmd_arg_map
